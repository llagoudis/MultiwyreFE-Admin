...
const EcommerceTransactions = () => {
  ...
  const columns = [
...

    {
      ...
      valueGetter: ({ row }: TableRow) => row?.requestedAmount,
      headerName: "FIAT AMOUNT",
      renderCell: ({ row }: TableRow) => (
        <span>{`${row?.requestedAmount ? row?.requestedAmount : "---"} `}</span>
      ),
    },

    {
      minWidth: 150,
      field: "requestedAssetId",
      valueGetter: ({ row }: TableRow) => row?.requestedAssetId,
      headerName: "FIAT CURRENCY",
      renderCell: ({ row }: TableRow) => (
        <span>{`${row?.requestedAssetId ? row?.requestedAssetId : "---"} `}</span>
      ),
    },
...

    {
      minWidth: 200,
      field: "fxMarkup",
      valueGetter: ({ row }: TableRow) => row?.fxMarkup,
      headerName: "FX MARKUP",
      renderCell: ({ row }: TableRow) => (
        <span>{`${row?.fxMarkup ? row?.fxMarkup : "---"} `}</span>
      ),
    },

    {
      minWidth: 200,
      field: "networkFee",
      valueGetter: ({ row }: TableRow) => row?.networkFee,
      headerName: "NETWORK FEE",
      renderCell: ({ row }: TableRow) => (
        <span>{`${row?.networkFee ? row?.networkFee : "---"} `}</span>
      ),
    },
    ...
    //   minWidth: 200,
    //   field: "exactAmount",
    //   valueGetter: ({ row }: TableRow) => row?.exactAmount,
    //   headerName: "CRYPTO AMOUNT",
    //   renderCell: ({ row }: TableRow) => (
    //     <span>{`${row?.exactAmount ? row?.exactAmount : "---"} `}</span>
    //   ),
    // },

    {
      minWidth: 150,
      field: "amount",
      valueGetter: ({ row }: { row: EcomTransactions }) => row?.amount,
      headerName: "RECEIVED CRYPTO",
      renderCell: ({ row }: { row: EcomTransactions }) => {
        ...
        const amountIsEmpty = (row?.amount ?? "---") === "---";
        color = amountIsEmpty ? "black" : color;
...

    {
      ...
      headerName: "CONVERSION CURRENCY",
    },
