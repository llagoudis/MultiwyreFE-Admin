...
import { fetchAdminCommissionWalletTrxs } from "~/service/ApiRequests";
import {
  ExportCsv,
  formatDateTime,
...
const TableComponentCommission = (reload: propType) => {
  ...
  const getReports = async (data: FilterType) => {
    ...
    }> = await ApiHandler(fetchAdminCommissionWalletTrxs, data);
    setLoading(false);
    if (error) {
  ...
  async function handleExport() {
    ...
    const [res, error]: APIResult<{
      data: Project[];
      pagination: Pagination;
    }> = await ApiHandler(fetchAdminCommissionWalletTrxs, paramsQuery);
...
export default TableComponentCommission;
