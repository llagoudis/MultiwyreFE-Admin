...
import { fetchBulkTransactions } from "~/service/ApiRequests";
import { ApiHandler } from "~/service/UtilService";

type TableRow = { row: BulkTransactions };
...
const BulkTransactions = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      hidden: true,
      renderCell: ({ row }: TableRow) => {
        return <span style={{ fontSize: "12px" }}>{row?.fileName}</span>;
      },
    },
    {
      field: "fromAddress",
  ...
  const getReports = async (data: FilterType) => {
    ...
    }> = await ApiHandler(fetchBulkTransactions, data);
  ...
  async function handleExport() {
    ...
    const [res, error]: APIResult<{
      data: BulkTransactions[];
      pagination: Pagination;
    }> = await ApiHandler(fetchBulkTransactions, paramsQuery);
    ...
    void ExportCsv(reportHeaderval, "BULK TRANSACTIONS");
  }
