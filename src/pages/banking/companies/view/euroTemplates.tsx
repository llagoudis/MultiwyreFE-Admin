...
const Accounts: React.FC<defaultCompanyProps> = ({ data }) => {
  const router = useRouter();

  const userId = data?.company?.UserCompanyAssociations[0]?.User?.azureId ?? "";
  ...
  useEffect(() => {
    ...
    if (userId) void getTemplatesById(userId);
  }, [userId]);
